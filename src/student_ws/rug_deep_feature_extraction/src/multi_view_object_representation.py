...
with graph.as_default():
    ...
    with session.as_default():
        ...
        elif (base_network == "ICRA2021"):
            autoencoder = load_model('/home/hamidreza/student_ws/rug_deep_feature_extraction/models/ICRA2021.hdf5')
            encoder = Model(inputs=autoencoder.input, outputs=autoencoder.get_layer('dropout_1').output)
...
def handle_orthographic_representation(req):
    ...
    for i in range(0, number_of_views):
		...
		#image_size = 300
                resized_img, othographic_image = preprocessingForOrthographicImages(img, image_size)
                img_g = cv2.merge((resized_img, resized_img, resized_img))

		cv2.imshow('RGB_image', img_g)
		cv2.waitKey(1)
