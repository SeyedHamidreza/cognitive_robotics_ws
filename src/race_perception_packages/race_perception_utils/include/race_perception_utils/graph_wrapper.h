...
class GVGraph
{
	public:
		...
		*/
		int setGraphAttribute(string attribute,string value);
		...
		*/
        ...
        void startRenderFP(FILE *out);

        void startRenderFile(char *filename);


		GVC_t*getGVCcontext(void);
		
		void _gvconfig_plugin_install_from_library(GVC_t*gvc,char *path, gvplugin_library_t *library);
