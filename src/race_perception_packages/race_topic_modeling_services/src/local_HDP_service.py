...
--------------------------------------------------------------------
"""
def predictionIsTrue(hdps,BoW,true_category,learnedCats):
    likelihoods=[]
    # for i in range(len(categories)):
    #     likelihoods.append(hdps[categories[i]].evaluate_test_corpus([BoW]))
    for hdp in hdps:
        likelihoods.append(hdp.evaluate_test_corpus([BoW]))
...
def initializeHDPModel(max_number_of_topics_for_hdp,num_vocab):
...
def testingPhase_HDP(ground_truth_label,BoW_representation):
    ...
    predictStatus, predictionCategory = predictionIsTrue(hdps, BoW, ground_truth_label, learned_categories)
    print("Prediction is: ", predictStatus)
    print("Predicted Label: ", predictionCategory)
...
def train_the_HDP_models(ground_truth_label,BoW_representation):
    ...
    hdps[learned_categories.index(ground_truth_label)] = HdpModel(corpus=BoWs[learned_categories.index(ground_truth_label)]
                                                                  , id2word=dictionary, chunksize=1, K=10,
                                                                    T=max_number_of_topics_for_hdp)
    for i in range(10):
        hdps[learned_categories.index(ground_truth_label)].update(BoWs[learned_categories.index(ground_truth_label)])
    return 0

def add_new_category(ground_truth_label):
    learned_categories.append(ground_truth_label)
    hdps.append(HdpModel(corpus=None, id2word=dictionary, chunksize=1, K=10,
                         T=max_number_of_topics_for_hdp))  # note that in gensim self.m_D is set to 1 by us
    # hdps.append(LdaModel(corpus=None, id2word=dictionary, chunksize=1,num_topics=30))  # note that in gensim slef.m_D is set to 1 by us
    BoWs.append([])
    return 0
...
def handle_HDP_representation_and_recognition(req):
    ...
    topic_info = hdps[learned_categories.index(ground_truth_label)].print_topics(num_topics=1, num_words=500)
    # print(topic_info)
...
max_number_of_topics_for_hdp = 100
dictionary_size = 500
num_learned_categories=0
learned_categories = []
BoWs=[]
hdps=[]
dictionary = initializeHDPModel(max_number_of_topics_for_hdp, dictionary_size)
local_HDP_server()
