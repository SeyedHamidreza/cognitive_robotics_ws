...
def topic_to_vector(topic):
    topic_vector = [0]*max_number_of_topics_for_hdp
    topic_probs = [prob for (x,prob) in topic]
    topic_indices= [x for (x,prob) in topic]
    for i in range(0,max_number_of_topics_for_hdp):
...
def handle_HDP_representation_and_recognition(req):
    ...
    topic_info = hdp.print_topics(num_topics=1, num_words=90)
    # print(topic_info)
...
hdp=HdpModel(corpus=[BoWs], id2word=dictionary, chunksize=1, K=10,T=max_number_of_topics_for_hdp)
local_HDP_server()
