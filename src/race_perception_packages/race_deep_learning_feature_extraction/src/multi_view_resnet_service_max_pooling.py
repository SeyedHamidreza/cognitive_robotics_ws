...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        resnet_feature_extraction = Model(inputs=resnet_model.input, outputs=resnet_model.get_layer('avg_pool').output)
...
#__________________________
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
         with session.as_default():    
            features = resnet_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = resnet_feature_extraction.predict(x_b)
                features_g = resnet_feature_extraction.predict(x_g)
                features_r = resnet_feature_extraction.predict(x_r)
...
def resnet50_server():
    ...
    s = rospy.Service('resnet50_service', deep_representation, handle_good_representation)
    print "Ready to representas based on resnet50 network."
    rospy.spin()
