...
with graph.as_default():
	...
	#mobilenet_feature_extraction = Model(inputs=mobilenet_model.input, outputs=mobilenet_model.get_layer('Logits').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        if (max_pooling) :
            ...
            ## appending
            # features = np.append(features_b, features_g, axis=1)
            # features = np.append(features, features_r, axis=1)
