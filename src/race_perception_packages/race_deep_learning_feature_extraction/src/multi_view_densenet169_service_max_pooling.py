...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        densenet169_feature_extraction = Model(inputs=densenet169_model.input, outputs=densenet169_model.get_layer('avg_pool').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
            with session.as_default():    
                features = densenet169_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = densenet169_feature_extraction.predict(x_b)
                features_g = densenet169_feature_extraction.predict(x_g)
                features_r = densenet169_feature_extraction.predict(x_r)
...
def densenet169_server():
    rospy.init_node('deep_learning_representation_server')
    s = rospy.Service('densenet169_server', deep_representation, handle_good_representation)
    print "Ready to representas based on densenet169 network."
    rospy.spin()
