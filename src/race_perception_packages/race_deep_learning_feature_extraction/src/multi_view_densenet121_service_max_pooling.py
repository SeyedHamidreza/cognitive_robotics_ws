...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        densenet121_feature_extraction = Model(inputs=densenet121_model.input, outputs=densenet121_model.get_layer('avg_pool').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
            with session.as_default():    
                features = densenet121_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = densenet121_feature_extraction.predict(x_b)
                features_g = densenet121_feature_extraction.predict(x_g)
                features_r = densenet121_feature_extraction.predict(x_r)
...
def densenet121_server():
    rospy.init_node('deep_learning_representation_server')
    s = rospy.Service('densenet121_server', deep_representation, handle_good_representation)
    print "Ready to representas based on densenet121 network."
    rospy.spin()
