...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        xception_feature_extraction = Model(inputs=xception_model.input, outputs=xception_model.get_layer('avg_pool').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
         with session.as_default():    
            features = xception_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = xception_feature_extraction.predict(x_b)
                features_g = xception_feature_extraction.predict(x_g)
                features_r = xception_feature_extraction.predict(x_r)
...
def xception_server():
    ...
    s = rospy.Service('xception_service', deep_representation, handle_good_representation)
    print "Ready to representas based on xception network."
    rospy.spin()
