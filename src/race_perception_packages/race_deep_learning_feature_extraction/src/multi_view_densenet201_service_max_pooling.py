...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        densenet201_feature_extraction = Model(inputs=densenet201_model.input, outputs=densenet201_model.get_layer('avg_pool').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
            with session.as_default():    
                features = densenet201_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = densenet201_feature_extraction.predict(x_b)
                features_g = densenet201_feature_extraction.predict(x_g)
                features_r = densenet201_feature_extraction.predict(x_r)
...
def densenet201_server():
    rospy.init_node('deep_learning_representation_server')
    s = rospy.Service('densenet201_server', deep_representation, handle_good_representation)
    print "Ready to representas based on densenet201 network."
    rospy.spin()
