...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        nasnet_large_feature_extraction = Model(inputs=nasnet_large_model.input, outputs=nasnet_large_model.get_layer('global_average_pooling2d_1').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
            with session.as_default():    
                features = nasnet_large_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = nasnet_large_feature_extraction.predict(x_b)
                features_g = nasnet_large_feature_extraction.predict(x_g)
                features_r = nasnet_large_feature_extraction.predict(x_r)
...
def nasnet_large_server():
    rospy.init_node('deep_learning_representation_server')
    s = rospy.Service('nasnet_large_server', deep_representation, handle_good_representation)
    print "Ready to representas based on nasnet_large network."
    rospy.spin()
