...
with graph.as_default():
    ...
    with session.as_default():    
        ...
        inception_resnet_feature_extraction = Model(inputs=inception_resnet_model.input, outputs=inception_resnet_model.get_layer('avg_pool').output)
...
def handle_good_representation(req):
    ...
    if (not multiviews): # make an image using three planes of good descriptor
        ...
        with graph.as_default():  
            with session.as_default():    
                features = inception_resnet_feature_extraction.predict(x)
        ...
        with graph.as_default():  
            with session.as_default():    
                features_b = inception_resnet_feature_extraction.predict(x_b)
                features_g = inception_resnet_feature_extraction.predict(x_g)
                features_r = inception_resnet_feature_extraction.predict(x_r)
...
def inception_resnet_server():
    rospy.init_node('deep_learning_representation_server')
    s = rospy.Service('inception_resnet_service', deep_representation, handle_good_representation)
    print "Ready to representas based on inception_resnet network."
    rospy.spin()

if __name__ == "__main__":
    inception_resnet_server()
